...
# ── CORS Middleware ───────────────────────────────────────────────────────
# Single CORS block only. allow_origins must be an explicit whitelist because
# allow_credentials=True cannot be combined with allow_origins=["*"] per the
# CORS spec — browsers will silently reject the response if you try.
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
async def preprocess_and_clean_endpoint(
    ...
    # Relative URLs — same-origin download routes, no env var needed.
    return {
        "status": "success",
        "total_rows_1s": len(df),
        "total_rows_1m": len(df_1m),
        "detected_flares": detected_flares,
        "download_nowcast_url": "/download/nowcast",
        "download_forecast_url": "/download/forecast",
...
def _process_live_point(sxr_raw: float, hxr_raw: float) -> dict[str, Any]:
    """Core live-point processing logic: cleans a single telemetry point, runs
    Nowcaster & Forecaster inference, logs it, and returns the result dict.

    This is a plain function (no HTTP, no async) so it can be called directly
    both from the /predict/live endpoint AND from the background simulation
    thread — no self-HTTP round trip, no silently-swallowed request errors.
    """
    global pipeline, live_solexs_records, live_helios_records

    if pipeline:
        cleaned_sxr_list, _ = pipeline.clean([sxr_raw], "solexs")
        cleaned_hxr_list, _ = pipeline.clean([hxr_raw], "helios")
    ...
    # Forecasting (tiered heuristic — matches injected flare lifecycle)
    forecast_class = "B2.1"
    hours = 12.0
    raw = sxr_raw
    if raw > 25.0:
...
async def predict_live_point(request: LivePointRequest) -> Any:
    """Ingest a single telemetry point, clean it, run inference, and log it."""
    return _process_live_point(request.sxr_raw, request.hxr_raw)
...
# ── WebSocket Telemetry Stream (independent demo stream, not the simulation) ─
...
def _run_simulation_thread(loop: asyncio.AbstractEventLoop) -> None:
    ...
    through quiet-sun and realistic M/X flare events, feeding each point
    directly through _process_live_point() in-process (no self-HTTP call),
    ...
    for sxr, hxr in packet_stream():
        ...
        tick += 1
        try:
            result = _process_live_point(round(sxr, 3), round(hxr, 3))
